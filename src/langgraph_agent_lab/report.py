...
def _format_errors(item: ScenarioMetric) -> str:
    if not item.errors:
        return "-"
    return "<br>".join(item.errors)
...
def render_report(metrics: MetricsReport) -> str:
    ...
    scenario_rows = "\n".join(
        (
            f"| {item.scenario_id} | {_yes_no(item.success)} | {item.expected_route} | "
            f"{item.actual_route} | {item.nodes_visited} | {item.retry_count} | "
            f"{_yes_no(item.approval_observed)} | {_format_errors(item)} |"
        )
    ...
    failed_summary = (
        ...
        if not failed
        else ", ".join(f"{item.scenario_id} ({item.actual_route})" for item in failed)
    )

    return f"""# Agent Lab Report
...
Current run result: **{metrics.success_rate:.2%} success rate** across **{metrics.total_scenarios} scenarios**. {failed_summary}
...
| Total approval steps | {metrics.total_interrupts} |
